...
// 1. Webhook Verification (Handshake for WhatsApp)
router.get('/webhook/whatsapp', (req, res) => {
    // This must match the waConfig.verifyToken from your React frontend
    const VERIFY_TOKEN = "ma_wa_handshake_your_unique_string"; 
    ...
    if (body.object === 'whatsapp_business_account') {
        try {
            ...
            if (value?.messages && value.messages[0]) {
                ...
                const msgText = message.text?.body || "Non-text content";

                console.log(`WhatsApp Message from ${from}: ${msgText}`);

                // --- INTEGRATION POINT FOR MYAUTOBOT ---
                // Here you would:
                // 1. Fetch User Config from DB (based on Phone ID in the payload)
                // 2. Deduct 5 tokens for this API call
                // 3. Call your self-hosted LLM on your VPS
                ...
                await axios.post(
                    `https://graph.facebook.com/v21.0/${PHONE_ID}/messages`,
                    {
                        ...
                        to: from,
                        type: "text",
                        text: { body: `MyAutoBot: Processing your request...` }
                    },
